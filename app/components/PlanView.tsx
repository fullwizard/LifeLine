  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-teal-200 bg-teal-50 p-5">
        <h2 className="text-lg font-semibold text-teal-950">Your plan</h2>
        <p className="mt-2 text-stone-800 leading-relaxed">{plan.summary}</p>
        {plan.steps.length > 0 && (
          <ol className="mt-4 space-y-1.5 text-sm text-stone-800">
            {plan.steps.map((s, i) => (
      ...
      <section>
        ...
        {plan.ranked.length === 0 ? (
          <p className="mt-4 rounded-lg border border-stone-200 bg-white p-4 text-sm text-stone-600">
      ...
      <p className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-900 leading-relaxed">
        {plan.disclaimer}
      </p>

      <button type="button" onClick={onReset} className="text-sm font-medium text-teal-700 hover:text-teal-900">
        ← Start over
      </button>
    </div>
  );
...
function Legend() {
  return (
    <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-600">
      ...
      <li className="flex items-center gap-1.5">
        <span className="h-2.5 w-2.5 rounded-full bg-stone-400" aria-hidden /> Not met yet
      </li>
    </ul>
  );
}
