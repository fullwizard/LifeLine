...
describe("keyword parser fallback", () => {
  it("extracts location, status, needs, children, and income", () => {
    const s = parseSituationByKeywords(
      "I live in Seattle with my two kids. Got a pay or vacate notice yesterday, I make about $2,400 a month and I'm behind on rent. Also our power might get shut off.",
    );
    expect(s.location?.county).toBe("King County");
  ...
  it("recognises veterans, ZIP codes, and yearly income", () => {
    const s = parseSituationByKeywords("Veteran in 98104, sleeping in my car, made 30k a year before I got laid off");
    expect(s.isVeteran).toBe(true);
    expect(s.location?.zip).toBe("98104");
