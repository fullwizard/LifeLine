...
function categoryFor(page) {
  ...
  const strongSignals = [
    ["rental_assistance", /\b(?:rental assistance|rent assistance|eviction (?:prevention|help)|housing (?:assistance|support|programs?)|homeless(?:ness)? assistance)\b/i],
    ["shelter", /\b(?:emergency shelter|temporary shelter|homeless shelter|shelter services?)\b/i],
...
function makeCandidate(page, source, generatedAt) {
  ...
  const name = clean(page.title, 180) || clean(page.headings?.[0], 180) || new URL(page.url).pathname;
...
export function normalizeCrawl(crawl) {
  ...
  for (const source of crawl.sources ?? []) {
    for (const page of source.pages ?? []) {
      ...
      seen.add(canonical);
      candidates.push(makeCandidate(page, source, generatedAt));
    }
  }
